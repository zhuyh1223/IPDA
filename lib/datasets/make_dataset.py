...
def make_dataset(cfg, dataset_name, transforms, is_train=True, split=None):
...
def make_data_loader(cfg, is_train=True, is_distributed=False, max_iter=-1, split=None):
    if is_train:
    ...
    dataset = make_dataset(cfg, dataset_name, transforms, is_train, split)
    sampler = make_data_sampler(dataset, shuffle)
    batch_sampler = make_batch_data_sampler(cfg, sampler, batch_size, drop_last, max_iter)
    num_workers = cfg.train.num_workers if is_train else 0
