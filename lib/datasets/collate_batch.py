...
def snake_collator(batch):
    ...
    if 'inp_dets' in batch[0]:
        inp_dets = default_collate([b['inp_dets'] for b in batch ])
        ret.update({'inp_dets': inp_dets})
    ...
    if ct_num != 0:
        wh[ct_01.bool()] = torch.Tensor(sum([b['wh'] for b in batch], []))
        ct_cls[ct_01.bool()] = torch.LongTensor(sum([b['ct_cls'] for b in batch], []))
        ct_ind[ct_01.bool()] = torch.LongTensor(sum([b['ct_ind'] for b in batch], []))
    ...
    if ct_num != 0:
        i_it_4pys[ct_01] = torch.Tensor(np.array(sum([b['i_it_4py'] for b in batch], [])))
        c_it_4pys[ct_01] = torch.Tensor(np.array(sum([b['c_it_4py'] for b in batch], [])))
        i_gt_4pys[ct_01] = torch.Tensor(np.array(sum([b['i_gt_4py'] for b in batch], [])))
        c_gt_4pys[ct_01] = torch.Tensor(np.array(sum([b['c_gt_4py'] for b in batch], [])))
    ...
    cls_gt_pys = torch.zeros([batch_size, ct_num, snake_config.gt_poly_num, 1], dtype=torch.float)
    if ct_num != 0:
        ...
        cls_gt_pys[ct_01] = torch.Tensor(np.array(sum([b['cls_gt_py'] for b in batch], [])))
